...
namespace Dziennik.Controllers
{
	public class NauczycielController : Controller
	{
		...
		public ActionResult PlikiPrzedmiotu([Bind(Include = "ID")] Przedmiot przedmiot, HttpPostedFileBase[] fileUpload)
		{
			...
			if (przedmiot.ID != null)
			{
				...
				Przedmiot original = db.Przedmioty
					.Include(p => p.Pliki.Where( plik => plik.NauczycielID == userId))
				...
				if (fileUpload != null)
				{
					foreach (var file in fileUpload)
					{
						...
						var newFile = new Plik { PrzedmiotID = przedmiot.ID, FilePath = sciezka, NauczycielID = userId, DataDodania = DateTime.Now };
		...
		private int IDPRZEDMIOTUWYBRANEGO;
		public ActionResult Klasy(int? id)
		{
			...
			var lvl = przedmiot.level.ToString();
            int userId = Int32.Parse((string)Session["UserID"]);


            var klasy = db.Klasy
                .Join(db.Lekcja.Where(l => l.NauczycielID == userId && l.PrzedmiotID == id),
                k => k.KlasaID,
                l => l.KlasaID,
                (k,l) => k)
                .Where(s => s.level.ToString().ToLower() == lvl.ToLower()).ToList();
		...
		public ActionResult Testy(int? id)
		{
			...
			var testy = db.Testy
				.Where(t => t.PrzedmiotID == id && t.NauczycielID == userId)
				.Include(t => t.Pytania)
